...
namespace InfluenceMaximization
{
	...
	public class Graph
	{
        ...
        public List<int> newreach(List<int> s, List<int> x)
        {
            List<int> nrlist = new List<int>();
            foreach(int u in s)
            {
                foreach (Node v in this.adj[u])
                {
                    if (!x.Contains(v.id) && !nrlist.Contains(v.id)) { nrlist.Add(v.id); }
                }
            }
            return nrlist;
        }
        public List<int> findlarge(List<int> range, int nums)
        {
            if (range.Count <= nums) return range;
            List<int> degrees = new List<int>();
            for (int i = 0; i < range.Count; i++) { degrees.Add(this.adj[range[i]].Count); }
            List<int> result = new List<int>();
            while (nums > 0)
            {
                int flag = 0;
                for (int i = 1; i < range.Count; i++) { if (degrees[i] > degrees[flag]) flag = i; }
                result.Add(range[flag]);
                degrees[flag] = -1;
                nums--;
            }
            return result;
