...
namespace InfluenceMaximization
{
	public class ICModel
	{
		...
		public Tuple<double, double> InfluenceSpread(Graph graph, List<int> seeds, List<double> prob, int R)
		{
			double sum = 0;
            ...
            for (int rnd = 0; rnd < R; ++rnd) 
			{
                ...
                {
                    if (random.NextDouble() < prob[i]) { realize.Add(seeds[i]); }
                }
				double sp = Propagation(graph, realize);
				sum += sp;
				spreads.Add(sp);
			}
			double ave = sum / R;
        ...
        public Tuple<double, double> InfluenceSpread(Graph graph, List<int> seeds, int R)
        {
            double sum = 0;
            Random random = new Random();
            List<double> spreads = new List<double>();
