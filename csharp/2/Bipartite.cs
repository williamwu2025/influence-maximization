	...
	public class Bipartite
	{
		...
		public Bipartite (string filepath, double alpha, int numV)
		{
			...
			for (int i = 0; i < numV; ++i) 
			{
                ...
                List<string> line = v2s.ReadLine().Replace("\n", "").Split().ToList();
                line.Remove("");
                foreach (string u in line) { vsets.Add(int.Parse(u)); }
                V2S.Add(vsets);
            }
            for (int i = 0; i < numS; ++i) 
			{
				List<int> snodes = new List<int> ();
                List<string> line = s2v.ReadLine().Replace("\n", "").Split().ToList();
                line.Remove("");
                foreach (string u in line) { snodes.Add(int.Parse(u)); }
                S2V.Add (snodes);
			}
        }

        //Greedy for influence maximization
		public Tuple<List<int>, double> Greedy(int k, List<int> init)
		{
